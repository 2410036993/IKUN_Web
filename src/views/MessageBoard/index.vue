...
import { getMessageList, addMessageUser, sendMessage } from '@/api/message'
/**
* Loading效果
*/
...
// 页面加载获取到
const getUser = () => {
  const jsonUser = window.localStorage.getItem("messageUser")
  // 判断
  if (jsonUser) {
    useMs.value = JSON.parse(jsonUser)
  } else {
    return
  }
}
...
const computedUser = (() => {
  // 高亮当前用户的状态
  const json = window.localStorage.getItem("messageUser")
  // 判断如果没有就终止该方法
  if (!json) {
    return
  }
  // 获取到唯一的头像
  const { userUrl } = JSON.parse(json)
  // 获取到当前用户的弹幕信息
  let danmus = danmusList.value.find(item => item.userUrl === userUrl)
  // 存储起来
  useDanums.value = danmus
})
// 页面加载获取到留言用户信息
onMounted(() => {
  // 页面刷新
...
const handleAvatarSuccess = (response) => {
  useMs.value.userUrl = response.data
}
// 记录是否是第一次发送消息
const timeOut = ref(0)
// 时空留痕保存
const confirmDialog = async () => {
  if (useMs.value.name.trim('') && useMs.value.userUrl) {
    // 发送留言用户
    await addMessageUser(useMs.value)
    ElMessage.success("可以发送留言了!")
    // 存储到localstroage中
    window.localStorage.setItem("messageUser", JSON.stringify(useMs.value))
    timeOut.value = 1
    // 关闭弹层
    visible.value = false
  } else {
...
const sendMs = async () => {
  ...
  if (useMs.value.name.trim('') && useMs.value.userUrl) {
    ...
    // 从后端获取到留言信息
    await sendMessage({ ...useMs.value, content: content.value })
    // 重新获取并刷新页面需要等待组件
    await getDanmusList()
    // 如果是第一次发送留言的话就得重新计算数据
    if (timeOut.value === 1) {
       computedUser()
    }
    timeOut.value++
    // 发送成功
    ElMessage.success('感谢你在我生命里留下痕迹')
    // 清除留言数据
    content.value = ''
  } else {
    // 用户没有输入数据打开留言弹层
...
<template>
  ...
  <el-dialog v-model="visible" style="background-color:#131f2b;" width="500px" :before-close="handleClose" center
    :show-close="false">
    <template #header>
      <span style="color: #3fa8db;font-weight: bold;font-size: 20px;">时空留痕</span>
    </template>
    <div class="avatar-box">
      <!-- 头像地方 -->
      <el-upload action="/prod-api/web/message/fileUpload" :show-file-list="false" class="avatar-uploader"
        style="height: 100px;width: 100px;border-radius: 50%;" :on-success="handleAvatarSuccess">
        <img v-if="useMs.userUrl" :src="useMs.userUrl" class="avatar" />
        <el-button v-else
          style="background-color: #131f2b;border: none;margin:33px 0px 0px 26px;font-size: 30px">+</el-button>
      </el-upload>
    </div>
    <el-input v-model="useMs.name" placeholder="为留言起个名字" clearable size="large" style="width: 220px;
      margin-left: 124px;background-color: #fff;margin-bottom: 6px;
    " class="lyinput"></el-input>
    <!-- 底部按钮 -->
    <template #footer>
  ...
  <div class="box-xk">
    ...
    <div class="biu" @keyup.enter="sendMs">
      <input type="text" v-model="content" placeholder="总要留下点什么才好" class="input">
      <i class="iconfont icon-fasongliuyan icon" @click="sendMs"> </i>
    </div>
    <!-- 弹幕列表 -->
    <Danmaku :danmus="danmusList" class="danmus" autoplay loop useSlot useSuspendSlot isSuspend randomChannel="true">
      <!-- 自定义弹幕的样式 -->
      <template v-slot:dm="{ danmu }">
        <div class="danmu-item">
          <img class="danmu-item--avatar" :src="danmu.userUrl" alt="">
          <div class="box-text"><span style="color:#aac9fa;">{{ danmu.name }}:</span>
            <!-- 这里进行判断如果是当前用户评论的则显示高亮提示用户 -->
            <span v-if="useDanums?.userUrl === danmu?.userUrl" style="color: #b0b2b5;font-weight: bold;">{{
    danmu.content }}</span>
            <!-- 初始化状态 -->
            <span v-else color="#8597b4">{{ danmu.content }}</span>
          </div>
        </div>
      </template>
      <!-- 鼠标悬浮后的样式 -->
